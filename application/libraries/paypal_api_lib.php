...
class Paypal_Api_Lib {
    ...
    public function __construct() {
        ...
        $this->add_nvp('COUNTRYCODE', $this->CI->config->item('paypal_lib_country_code'));
    }

    public function add_nvp($field, $value) {

        $this->nvps[$field] = urlencode($value);
    }

    public function send_api_call($method) {
    ...
    public function display_nvp_errors() {
    ...
    public function return_nvp_errors() {
    ...
    public function log_nvp_results($success) {
        if (!$this->do_log)
            return;
